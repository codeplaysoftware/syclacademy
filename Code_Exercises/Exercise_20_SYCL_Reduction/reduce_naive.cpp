...
int main(int argc, char *argv[]) {
  ...
  auto e2 = q.submit([&](sycl::handler &cgh) {
    ...
    cgh.parallel_for<class step1_reduce>(myNd, [=](sycl::nd_item<1> item) {
      ...
      item.barrier();
      workgroup_reduce_local_mem<T, workGroupSize>(local_mem, item, localIdx);
  ...
  auto e3 = q.submit([&](sycl::handler &cgh) {
    ...
    auto second_nd =
        sycl::nd_range(sycl::range(workGroupSize), sycl::range(workGroupSize));

    // Reduce smaller array into a single value
    cgh.parallel_for<class step2_reduce>(second_nd, [=](sycl::nd_item<1> item) {
      auto localIdx = item.get_local_linear_id();
      local_mem[localIdx] = 0;
  ...
  T devAns = 0;
  q.memcpy(&devAns, devPtrB, sizeof(T), e3).wait();
